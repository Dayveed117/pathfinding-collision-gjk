...
SquareCollider::SquareCollider(Transform* t)
{
    this->t = t;
}

glm::vec3 SquareCollider::FindFurthestPoint(const vec3 direction)
{
    // Initial point from polygon
    // Max initial point from polygon, projected in the world
    // Max distance from distance
    auto vertexi = Square::GetVertices();
    auto model   = t->model;
    ...
    for (size_t i = 0; i < vertexi.size(); i+=6)
    {
        ...
        vec3 world_point = model * vec4(point, 1.0f);
